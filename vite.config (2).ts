...
function oasisSmsProxyPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/sms/send', (req, res, next) => {
        ...
        req.on('end', async () => {
          try {
            ...
            const cleanKey = (body.apiKey || body.api_key || '').trim();
            const sender = (body.from || body.sender || body.sender_id || '').trim() || 'AHC MKONONI';
            const rawPhone = Array.isArray(body.to) ? body.to[0] : (body.to || body.recipient || body.mobile || body.phone || '');
            const messageText = body.text || body.message || body.body || '';
            const userTargetUrl = body.baseUrl || 'https://bulksms.oasistech.co.tz/api/sms';
