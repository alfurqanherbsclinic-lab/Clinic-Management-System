...
import {
  ...
  registerStaffBiometricCredential,
  authenticateStaffBiometricLogin,
  StaffMember
...
// Orodha ya awali ya Wafanyakazi wa Ofisi
...
export function LoginScreen({ onLoginSuccess, initialAttempts, onIncrementAttempts }: LoginScreenProps) {
  ...
  // WebAuthn Hardware Compatibility state
  ...
  const [regCredential, setRegCredential] = useState<any>(null);
  const [regSuccessMsg, setRegSuccessMsg] = useState("");
  const [regErrorMsg, setRegErrorMsg] = useState("");

  // Biometric Login States (Udhibiti wa WebAuthn Fingerprint)
  const [biometricScanning, setBiometricScanning] = useState(false);
  const [biometricStatus, setBiometricStatus] = useState("");
  const [scanSuccess, setScanSuccess] = useState(false);
  const [biometricError, setBiometricError] = useState("");

  // Angalia uwezo wa WebAuthn wa kifaa
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      unsubscribe = onSnapshot(collection(db, "staff_members"), (snapshot) => {
        if (!snapshot.empty) {
          const staffList = snapshot.docs.map(docSnap => docSnap.data() as StaffMember);
          ...
          if (!isInitialized) {
            // Seed initial default staff ONLY ONCE on very first launch of fresh DB
            localStorage.setItem("al_furqan_staff_initialized", "true");
            ...
            // User intentionally deleted all staff members
            setRegisteredStaff([]);
  ...
  // Usajili wa Real WebAuthn Biometric / Fingerprint ya kifaa
  const triggerWebAuthnRegistration = async () => {
    if (!regName.trim() || !regUsername.trim()) {
      setRegErrorMsg("Tafadhali jaza Jina Kamili na Username kwanza kabla ya kusajili Fingerprint.");
      return;
    }
    ...
    try {
      // Wito wa WebAuthn API kupitia biometricService.ts
      const credential = await registerStaffBiometricCredential(regUsername, regName);
      setRegCredential(credential);
      setRegScanning(false);
      setRegSuccessMsg("✅ Alama ya kidole cha kifaa hiki imerekodiwa kikamilifu kupitia WebAuthn!");
    } catch (err: any) {
      setRegScanning(false);
      setRegErrorMsg(err.message || "Usajili wa kidole cha WebAuthn ulifeli.");
    }
  };

  // Hifadhi usajili mpya wa mfanyakazi kwenye Firebase Firestore Database
  const handleRegisterStaffSubmit = async (e: React.FormEvent) => {
    ...
    if (!regName || !regUsername || !regPassword) {
      setRegErrorMsg("Tafadhali jaza taarifa zote za mfanyakazi kabla ya kuhifadhi.");
      return;
    }
    ...
    const newStaff: StaffMember = {
      ...
      webauthnCredential: regCredential || undefined
    };
    ...
    setRegSuccessMsg(`Hongera! ${regName} amesajiliwa kikamilifu kwenye Database.`);
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const dbStaff = registeredStaff.find(s => s.username.toLowerCase() === username.trim().toLowerCase());
  ...
  // Real WebAuthn Hardware Biometric / Fingerprint Login Execution
  const handleBiometricLogin = async () => {
    ...
    setBiometricStatus("Inafungua kihisi cha Fingerprint cha kifaa chako...");

    try {
      // Tuma wito wa kuthibitisha kidole halisi kupitia WebAuthn
      const authenticatedStaff = await authenticateStaffBiometricLogin(username, registeredStaff);

      setBiometricScanning(false);
      setScanSuccess(true);
      setBiometricStatus(`✅ UHAKIKI WA KIDOLE UMEFANIKIWA! Karibu ${authenticatedStaff.name} (${authenticatedStaff.roleDisplay})`);

      setTimeout(() => {
        onLoginSuccess(authenticatedStaff.username, authenticatedStaff.roleDisplay);
      }, 1000);
      ...
      setScanSuccess(false);
      onIncrementAttempts();
      setBiometricError(err.message || "❌ Uhakiki wa Fingerprint ulighairiwa au kidole hakikutambuliwa na kifaa.");
      setBiometricStatus("");
    }
  };
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200 overflow-hidden my-auto flex flex-col justify-between relative z-10">
        ...
        {activeTab === "login" ? (
          <div className="grid grid-cols-1 md:grid-cols-12 flex-1">
            ...
            <div className="md:col-span-7 p-5 sm:p-8 flex flex-col justify-between border-b md:border-b-0 md:border-r border-slate-100">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder="Demo: abdu.khalifa au daktari au famasia"
                          required
                        />
                      </div>
                    </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 text-left flex justify-between items-center">
                <span className="text-[9px] text-slate-400 font-bold font-mono">
                  IP: 192.168.1.102 • WebAuthn Ready
                </span>
                <span className="text-[9px] text-slate-400 font-bold font-mono">
                  SSL / HTTPS Active
                </span>
            ...
            {/* Right Section: Real WebAuthn Hardware Fingerprint Login Hub (5 columns) */}
            <div className="md:col-span-5 bg-slate-50 p-8 flex flex-col justify-between items-center text-center relative">
              ...
              <div className="w-full">
                <div className="flex justify-center mb-1">
                  <span className="bg-[#D6145A]/10 text-[#D6145A] text-[9px] font-extrabold px-2.5 py-1 rounded-full uppercase tracking-wider flex items-center gap-1">
                    <Smartphone className="w-3 h-3" /> Real Device Hardware WebAuthn
                  </span>
                </div>
                <h4 className="text-xs font-extrabold text-slate-700 uppercase tracking-widest mb-1">INGIA KWA FINGERPRINT HALISI</h4>
                <p className="text-[11px] text-slate-500 font-medium max-w-[220px] mx-auto leading-relaxed">
                  Gusa kitufe cha fingerprint hapo chini ili kufungua kihisi cha kidole cha simu, laptop au PC yako.
                </p>
              </div>

              {/* Hardware Biometric scanner box */}
              <div className="my-6 relative flex flex-col items-center">
                ...
                <button
                  ...
                  title="Gusa hapa ili kusoma Fingerprint halisi ya Kifaa chako"
                >
                  {biometricScanning && (
                    <>
                ...
                <span className="text-[10px] font-black text-slate-600 mt-3 tracking-wider uppercase select-none flex items-center gap-1">
                  Ingia kwa Fingerprint
                </span>

                <div className="flex items-center gap-2 mt-2 bg-white px-3 py-1.5 rounded-full border border-slate-100 shadow-sm">
                  ...
                  <span className="text-[9px] font-black tracking-wider text-slate-500 uppercase">
                    {biometricScanning ? "SCANNING..." : scanSuccess ? "PASSED!" : "WEBAUTHN READY"}
                  </span>
                </div>
              </div>
              ...
              <div className="w-full px-4 min-h-[60px] flex items-center justify-center">
                {biometricError ? (
                  ...
                  <p className="text-[10px] text-slate-400 font-bold max-w-[220px] leading-relaxed">
                    Acha au weka username, kisha gusa nembo ya <span className="text-[#D6145A]">Fingerprint</span> ili kuanza uhakiki wa kihisi cha kifaa chako.
                  </p>
                )}
              </div>
            ...
            <div className="max-w-3xl mx-auto w-full space-y-6">
              ...
              <div className="text-center">
                <span className="px-3 py-1 bg-[#D6145A]/10 text-[#D6145A] text-[10px] font-black rounded-full uppercase tracking-wider">
                  🔐 Usajili wa Ndani ya Mfumo (Admin Authorized Panel)
                </span>
                <h3 className="text-2xl font-black text-slate-800 tracking-tight mt-2">Usajili wa Mfanyakazi Mpya & WebAuthn Fingerprint</h3>
                <p className="text-xs text-slate-500 font-medium">Sajili taarifa za mfanyakazi na uhusishe fingerprint halisi ya kifaa chake (Passkey).</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                ...
                <form onSubmit={handleRegisterStaffSubmit} className="space-y-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">
                ...
                {/* Right registration real device biometric scanner box */}
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center text-center space-y-4">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    WEBAUTHN BIOMETRIC SENSOR
                  </span>
                  
                  <div className="relative">
                    <button
                      type="button"
                      onClick={triggerWebAuthnRegistration}
                      ...
                      title="Sajili Fingerprint Halisi ya Kifaa Hiki"
                    >
                      {regScanning && (
                  ...
                  <div>
                    <h5 className="text-xs font-extrabold text-slate-700">
                      {regScanning 
                        ? "Inafungua sensor ya kifaa chako..." 
                        : regCredential 
                        ? "✅ FINGERPRINT / PASSKEY IMEREKODIWA!" 
                        : "SAJILI FINGERPRINT YA KIFAA"}
                    </h5>
                  </div>

                  <p className="text-[10px] text-slate-400 max-w-[200px] leading-relaxed">
                    Gusa kitufe hapo juu ili kusajili Fingerprint / Passkey ya simu au kompyuta yako kwa usalama kupitia WebAuthn.
                  </p>
                </div>
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-emerald-700 font-bold flex items-center gap-1 mt-0.5">
                          ...
                          <span className="truncate">
                            {staff.webauthnCredential ? "WebAuthn Passkey Registered" : "Passkey Hardware Ready"}
                          </span>
                        </p>
                      </div>
